...
#### 1-1. 기존 엔드포인트 의미는 **반드시 코드를 읽고** 옮긴다

이미 존재하는 엔드포인트를 계약에 인용할 때는 **추측 금지**. 다음 두 곳을 직접 읽고 의미를 옮긴다:
- `apps/web/app/api/v1/.../route.ts` — 핸들러 본문, 사용 schema, 응답 형식
- `apps/web/lib/state-machine.ts` — 상태 전이가 걸린 엔드포인트는 허용 전이를 명시적으로 본다

이름만 보고 추측한 의미를 계약에 적으면 FE가 그 추측대로 와이어업하고, 사용자가 클릭하면 BE가 `INVALID_TRANSITION` 같은 에러를 반환한다 (R2 `/release` 사고: 이름은 "release"인데 실제로는 결재 상신 — PM이 "잠금 해제"로 추측해서 적었음).

추측이 불가피한 경우(코드를 읽어도 모호) 계약에 **`?` 또는 `TBD` 표시 + BE agent에게 명시적 질의 카드를 추가**한다. "아마", "~인 듯" 같은 어휘를 계약에 쓰지 않는다.
