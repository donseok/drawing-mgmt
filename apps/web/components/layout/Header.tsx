...
export function Header({ user }: HeaderProps) {
  ...
  return (
    <header
      className={cn(
        'sticky top-0 z-40 flex h-14 items-center gap-3 border-b border-border bg-bg/95 px-4 backdrop-blur',
      )}
    >
      <Link
        href="/"
      ...
      >
        <span
          aria-hidden
          className="inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-md bg-fg text-bg shadow-sm"
        >
          <span className="text-[11px] font-bold leading-none">DG</span>
        </span>
        <span className="hidden min-w-0 flex-col leading-tight md:flex">
          <span className="truncate text-[13px] font-semibold">동국씨엠 도면관리</span>
          <span className="truncate text-[11px] font-medium text-fg-subtle">Engineering Document Control</span>
        </span>
      </Link>
      ...
      <div className="mx-1 flex flex-1 items-center justify-center">
        ...
        >
          ...
          <span className="hidden text-xs text-fg-subtle lg:inline">문서·폴더·명령</span>
