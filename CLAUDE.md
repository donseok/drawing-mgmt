...
| 2026-04-27 | R33 designer 사고(main HEAD 직접 ff됨) + R34 pnpm-lock + 동일 파일 동시 작성 충돌 봉합 패턴 정착 | drawing-mgmt-team SKILL.md | R33 designer가 main HEAD를 직접 진행시킴 → spec이 어쨌든 main에 들어가서 R33 진행엔 영향 없었으나 격리 위반. R34에서 backend+viewer 둘 다 `@aws-sdk/client-s3` 추가로 pnpm-lock 충돌 + backend+frontend가 같은 `__tests__/storage-local.test.ts` 동시 작성으로 add/add 충돌. 봉합 패턴: (a) `git checkout --theirs pnpm-lock.yaml` + `pnpm install --no-frozen-lockfile`로 lock 재생성, (b) 동일 파일 add/add는 더 풍부한 쪽(보통 backend) 우선 `git checkout --ours`. 두 패턴은 R35에서 재사용해 깔끔히 통합. 결론: 같은 라운드에 같은 의존성/파일을 두 명에게 분배할 때 contract에 단일 소유자 명시 + 다른 측은 structural typing 또는 forward-compat 패턴(R34 viewer/R35 backend가 채택)으로 의존 우회. |
