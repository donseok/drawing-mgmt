...
비밀번호 정책 (PRD §4.1 FR-AUTH-03 + R39 A-4 강화)

- 최소 8자, 영문 + 숫자 + 특수문자 1개 이상, 공백 금지.
- 직전 2개 비밀번호 재사용 금지.
- 본인 정보(사번·생일 등) 사용 금지.
- 변경 폼은 강도 미터 + 체크리스트가 실시간 갱신.
- **만료 90일** (`PASSWORD_EXPIRY_DAYS` env). 만료 7일 전부터 페이지 상단 배너 알림.
  만료 후엔 `/settings?tab=password`로 강제 이동, 다른 페이지 접근 차단.
- 관리자가 강제 만료 가능 — `/admin/users` 상세 → `[비밀번호 만료]`.
...
- **복구 코드 재발급**: `/settings` → 보안 → `[복구 코드 재발급]` → 비밀번호 또는 6자리 재인증 → 신규 10개 표시.
- **비활성화**: `/settings` → 보안 → `[2단계 비활성화]` → 비밀번호 또는 6자리 재인증.

#### 1.2.5 사용자 설정 (`/settings`)

- **비밀번호 변경:** `현재 PW` → `새 PW` → `새 PW 확인` → `[변경]`
- **서명 이미지 업로드:** 결재 시 본인 서명으로 사용 (jpg/png, 1MB 이내 권장)
- **알림 환경설정:** 메일 / SMS / 카카오 알림톡 채널 토글 (R38 N-2). SMS·카카오는 `phoneNumber` 입력 필요.
...
- MFA 설정 컴포넌트: `apps/web/components/settings/MfaSection.tsx`
- MFA 로그인 페이지: `apps/web/app/(auth)/login/mfa/page.tsx`
...
#### 2.2.11 PDF 본문 전문 검색 (R40 S-1)

자료 첨부에 PDF가 있으면 본문 텍스트가 자동 추출되어 검색에 포함된다. 검색 결과 행에 매칭된 본문 단편이 `<mark>강조된 부분</mark>`으로 노출(1줄, 자동 truncate). 검색 placeholder는 "도면번호, 자료명, **PDF 내용** 검색…"으로 갱신됐다.

#### 2.2.12 검색 ranking 통합 (R42)

자료번호/이름/설명(trgm) + PDF 본문(FTS) 매칭 점수를 통합해 가장 관련도 높은 결과가 위로. PDF 본문 매칭에 약간 가중치(`FTS_WEIGHT=1.5`) — 본문에 정확히 있는 키워드가 메타에 약하게 있는 것보다 우선. 결과 행 옆 작은 칩으로 매치 출처 표시: **"본문"** / **"본문+메타"** / 메타만은 미표시.

#### 2.2.13 멀티 fragment 스니펫 + cursor 페이지네이션 (R43)

- 긴 PDF에서 매칭이 여러 곳에 있을 때 최대 3개의 컨텍스트 단편이 ` … ` 구분자로 연결되어 노출. 각 fragment는 15단어 안팎으로 짧게.
- "더 보기" 페이지로 이동해도 ranking이 일관 유지(이전엔 첫 페이지만 ranking 적용). 깊은 페이지 진입 시 권한 필터 후에도 가능한 한 limit만큼 결과 보장.
...
#### 6.5.7 선 가중치 / 속성창 (FR-VIEW-13 + R37 V-2)

우측 패널 토글로 선 가중치 표시 ON/OFF, 도면 entity 속성 창 호출. DXF 그룹 코드 370(LineWeight)을 Line2로 정확 렌더링하며, 인쇄/PDF 출력에 동일 두께가 반영된다(R37 V-2).
...
- **알림이 안 옴:** `/settings`에서 알림 환경설정(채널 on/off) 확인. SMS·카카오는 운영 환경 변수가 활성화돼야 발송된다.
- **알림 종류:** 위 10.4 표 참조.
...
| 2026-04-29 | **MANUAL-V03 v0.3 통합 리팩터**: 챕터 12를 본문에 분배 후 제거. 1.2(SAML 1.2.3 / MFA 1.2.4 / 비밀번호 정책 expansion) / 2.2(PDF FTS·ranking·fragment 2.2.11~13) / 6.5(측정 정밀도 6.5.3 / 선 가중치 6.5.7) / 10(알림 채널 10.5) 흡수. 관리자 페이지 catch-up은 부록 G로, 접근성 catch-up은 부록 H로 분리(reference 성격). |

---

## 부록 G. 관리자 페이지 (R36 / R40 / R41)

> 운영 일상에서 슈퍼관리자/관리자가 자주 보는 모니터링 페이지의 요약. 자세한 운영 절차는 `docs/manuals/operations.md` 참조.

### G.1 의존성 보안 (`/admin/security`, R40 + R41)
...
- **R-AUDIT-TREND(2026-04-29)**: 일일 cron snapshot이 `SecurityAuditSnapshot` 테이블에 누적되어 추세 비교 가능. `GET /api/v1/admin/security/audit/trend?days=N` (FE 차트는 후속 polish 카드).

### G.2 PDF 본문 추출 모니터 (`/admin/pdf-extracts`, R41)
...
- **재시도**: FAILED 또는 SKIPPED 행만 활성. ConfirmDialog 확인 후 큐에 재 enqueue. optimistic으로 PENDING 표시 후 5초 폴링으로 갱신.

### G.3 바이러스 스캔 모니터 (`/admin/scans`, R36)
...
> 이외 admin 페이지(`/admin/users`, `/admin/conversions`, `/admin/backups`, `/admin/storage`, `/admin/folder-permissions`, `/admin/groups`, `/admin/organizations`, `/admin/classes`)는 본문 챕터 1·4·5·8 및 운영 매뉴얼에서 다룬다.
...
- `focus-visible` ring + ARIA `role`/`aria-label` + 색대비 4.5:1 충족.
- 스크린 리더 호환 강화 — `<dl>` 시맨틱, `<mark>`/`<th aria-sort>` 등.
- 키보드 단축키 일람: 부록 A. 단축키만으로 전 페이지 도달 가능.
