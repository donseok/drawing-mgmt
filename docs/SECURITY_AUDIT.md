...
4. ~~**FIND-014**~~ — ✅ **R47 Mitigated.** authorize 첫 줄에 IP 기반 5/min rate limit + `LoginRateLimitedError(code='rate_limited')`. login-form.tsx mapErrorCode FE 매핑은 다음 polish 라운드.
5. **FIND-018** — LOGIN_SUCCESS/FAIL ActivityLog 기록.
6. **FIND-019** — 다운로드/프린트 ActivityLog 기록.
7. ~~**FIND-020**~~ — ✅ **R47 Mitigated** (FIND-002와 묶음 — `PASSWORD_CHANGE_SELF` ActivityLog 추가).
