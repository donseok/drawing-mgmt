...
| CSP `script-src` | OK (R-CSP) | `'self' 'nonce-{X}' 'strict-dynamic'` (dev에서만 `'unsafe-eval'` 유지). 미들웨어가 요청마다 `crypto.randomUUID()`로 nonce 생성 → `x-nonce` request header로 RSC/route 전달 → next-themes inline script + `/api/docs` swagger script/style에 매칭 nonce 부착. `'unsafe-inline'` 제거 완료. **FIND-015 Mitigated.** |
...
| 결과 기록 | OK — 일일 cron snapshot이 `SecurityAuditSnapshot`에 영속화, 1년치 trend 보관 (R-AUDIT-TREND, 2026-04-29). 인스턴스 재시작 무관. ~~FIND-016~~ Mitigated. |
...
- **심각도:** Medium
- **영역:** XSS 방어심층화
- **위치:** `apps/web/middleware.ts` (R-CSP 이전 line 106)
...
- **우선순위:** P2
- **상태:** Mitigated (R-CSP, 2026-04-29) — 미들웨어가 요청마다 `crypto.randomUUID()`로 nonce 생성하여 `x-nonce` request header로 RSC/route handler에 전달. `applySecurityHeaders(res, nonce)` 시그니처 확장 + `buildCsp(nonce)` 빌더로 `script-src 'self' 'nonce-{X}' 'strict-dynamic'` 발행 (dev는 HMR 호환을 위해 `'unsafe-eval'` 유지, prod는 제거). `app/layout.tsx`가 `headers().get('x-nonce')`로 읽어 `ThemeProvider`에 prop 전달, `next-themes` 0.3.0이 inline bootstrap script에 forward. `/api/docs`는 inline `<style>` + `<script>` 두 개에 nonce 부착. dev curl 검증으로 header CSP nonce와 body의 모든 `<script nonce>` 값 일치 확인. `'unsafe-inline'` 완전 제거. `style-src 'unsafe-inline'`은 React `style={...}` 21건 잔존으로 별 라운드(Phase 2)로 이연.
...
- **우선순위:** P3
- **상태:** Mitigated (R-AUDIT-TREND, 2026-04-29) — `SecurityAuditSnapshot` 모델 + manual 0016 마이그레이션 신설. `apps/worker/src/security-audit-worker.ts`가 BullMQ `security-audit` 큐를 소비, `pnpm audit --json` subprocess(GPL 격리 유지)를 90초 cap으로 실행하고 4개 severity counts + total + durationMs + 32KB cap advisoriesJson을 row에 INSERT. `SECURITY_AUDIT_CRON_ENABLED=1`이면 매일 02:30 UTC repeatable 등록 (BACKUP_CRON 02:00과 IO 분리). `GET /api/v1/admin/security/audit/trend?days=30&source=cron`이 ADMIN/SUPER_ADMIN에게 시계열을 반환, `POST /api/v1/admin/security/audit/snapshot`이 SUPER_ADMIN에게 즉시 큐 push 핸들 제공. 기존 `audit/route.ts`는 helper(`parseAuditDetail` 등)를 `@drawing-mgmt/shared/pnpm-audit-parser`로 import 경로만 바꿈 — 응답 shape / 캐시 동작 100% 보존. `SECURITY_AUDIT_RETENTION_DAYS=365`로 1년 cap. FE 차트 UI는 후속 polish 카드.
...
### P2 처리 현황 (R49 + R50 + R-CSP)

R49 + R50 + R-CSP에서 P2 묶음 처리:
...
- ~~**FIND-015**~~ CSP `script-src` nonce 마이그 ✅ (R-CSP, 2026-04-29) — `'unsafe-inline'` 제거, `'nonce-{X}' 'strict-dynamic'` 도입.
- ~~**FIND-021**~~ HSTS 헤더 ✅ (R49)
- ~~**FIND-022**~~ 백업 파일 암호화 ✅ (R50)

**모든 P2 Mitigated.** `style-src 'unsafe-inline'`은 React `style={...}` 21건 잔존으로 별 라운드(Phase 2)로 이연 — FIND 카드 별도 발행 시점에 처리.
...
| 2026-04-29 | R-CSP — 잔여 P2 1건(FIND-015) Mitigated. backend single round로 (1) `middleware.ts`에 `crypto.randomUUID()` per-request nonce 생성 + `x-nonce` request header forward + `applySecurityHeaders(res, nonce)` 시그니처 확장 + `buildCsp(nonce)` 빌더 (`script-src 'self' 'nonce-{X}' 'strict-dynamic'`, prod에서 `'unsafe-inline'`/`'unsafe-eval'` 제거, dev는 HMR용 `'unsafe-eval'` 유지) (2) `app/layout.tsx`가 `headers().get('x-nonce')` 읽어 `ThemeProvider` prop 전달 (3) `ThemeProvider`가 `next-themes` `nonce` prop forward (0.3.0 dist 검증) (4) `/api/docs` route가 `headers()`로 nonce 읽어 inline `<style>` + `<script>` 두 개에 부착. `font-src`에 `https://cdn.jsdelivr.net` 추가(Pretendard variable font fetch 명시 허용). 4 files. typecheck/build pass(50개 라우트 unchanged). dev curl 검증: header CSP nonce와 body의 모든 `<script nonce>`(Next 14 chunk + next-themes inline bootstrap)가 정확히 일치. **모든 P2 해소.** `style-src 'unsafe-inline'`은 React inline style 21건 잔존으로 별 라운드 이연. | 잔여 P2 마감 |
