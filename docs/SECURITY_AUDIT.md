...
### P1 (운영 시작 직후 빠른 후속 라운드) — **R48에서 대부분 Mitigated**

1. ~~**FIND-005**~~ — ✅ **R48 Mitigated.** `(main)/layout.tsx`에서 server-side check + `/settings|/logout` skip + 만료 시 forced redirect. middleware의 `x-pathname` 헤더 forwarding 추가.
2. ~~**FIND-006**~~ — ✅ **R48 Mitigated.** admin reset-password가 `validatePassword` 검증 + `passwordChangedAt: new Date(0)` 즉시 만료.
3. **FIND-009** — auth.config.ts에 trustHost + useSecureCookies 명시. (잔여)
4. ~~**FIND-014**~~ — ✅ R47 Mitigated. R48에서 FE login-form mapErrorCode `rate_limited` 한국어 매핑 추가.
5. ~~**FIND-018**~~ — ✅ **R48 Mitigated.** authorize의 모든 분기에서 LOGIN_SUCCESS/LOGIN_FAIL + metadata 기록.
6. ~~**FIND-019**~~ — ✅ **R48 Mitigated.** file/preview.dxf/preview.pdf/print 4개 라우트에 OBJECT_DOWNLOAD/PREVIEW/PRINT 기록(thumbnail은 정책상 skip).
7. ~~**FIND-020**~~ — ✅ R47 Mitigated.

**잔여 P1: FIND-009만.** 다음 라운드(R49) 또는 운영 시작 직후 hotfix.
