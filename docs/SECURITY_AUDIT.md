...
1. **FIND-001** — 모든 mutating route를 `withApi`로 마이그레이션. 즉시 작업하라. 누락 시 CSRF/CSRF-rate limit 회귀 가드를 ESLint custom rule 또는 grep CI step으로 추가.
2. **FIND-002** — me/password 라우트를 `validatePasswordWithHistory` + `buildPasswordChangeUpdate` 사용으로 재작성. 단위 테스트 3건(정책 위반/재사용/정상) 동봉.
3. **FIND-003** — middleware `isDemoPublic`에서 attachments 제거 + 각 attachment 라우트 `requireUser` + canAccess 가드 추가.
4. **FIND-004** — `auth.ts findDevUser`를 `NODE_ENV === 'production'` 강제 차단으로 보강 + `.env.example` 가이드 보강.

### P1 (운영 시작 직후 빠른 후속 라운드)

1. **FIND-005** — 90일 만료 enforce. 가장 단순한 방법은 RSC `app/(main)/layout.tsx`에서 server-side check + redirect.
2. **FIND-006** — admin reset-password에 정책 검증 + 즉시 만료.
3. **FIND-009** — auth.config.ts에 trustHost + useSecureCookies 명시.
4. **FIND-014** — Credentials authorize에 IP 기반 rate limit 추가.
5. **FIND-018** — LOGIN_SUCCESS/FAIL ActivityLog 기록.
6. **FIND-019** — 다운로드/프린트 ActivityLog 기록.
7. **FIND-020** — 비밀번호 변경 ActivityLog 기록 (FIND-002와 묶음).

P2/P3는 본 보고서를 백로그(`docs/잔여작업.md`)로 옮겨 분기 단위로 처리한다.
